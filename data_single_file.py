...
class Data:
    def __init__(self, dir_path):
        ...
        self.file_dict = {
            ...
            'eval': self.files[int(len(self.files) * 0.8):],
    ...
    def batch(self, batch_size, length, mode='train', batch_no = 0):

        #batch_files = random.sample(self.file_dict[mode], k=batch_size)
        batch_files = self.file_dict[mode][batch_no * batch_size:batch_no * batch_size + batch_size]
    ...
    def slide_seq2seq_batch(self, batch_size, length, mode='train', batch_no=0):
        data = self.batch(batch_size, length + 1, mode, batch_no)
        x = data[:, :-1]
        y = data[:, 1:]
        return x, y
