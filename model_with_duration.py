...
class MusicTransformer(torch.nn.Module):
    def __init__(self, embedding_dim=256, vocab_size=388+2, num_layer=6,
        ...
        self.fc_combined1 = torch.nn.Linear(self.vocab_size + 128, self.vocab_size + 128)
        self.fc_combined2 = torch.nn.Linear(self.vocab_size + 128, self.vocab_size)
    ...
    def forward(self, x, length=None, writer=None):
        if self.training or not self.infer:
            ...
            fc = self.fc(decoder)
            fc = torch.cat((fc, duration_info.to(decoder.device)), 2)
            fc = self.fc_combined1(fc)
            fc = self.fc_combined1(fc)
            fc = self.fc_combined1(fc)
    ...
    def generate(self,
        ...
        for i in Bar('generating').iter(range(length)):
            ...
            fc = self.fc(result)
            fc = torch.cat((fc, duration_info.to(result.device)), 2)
            fc = self.fc_combined1(fc)
            fc = self.fc_combined1(fc)
            fc = self.fc_combined1(fc)
            result = self.fc_combined2(fc)
